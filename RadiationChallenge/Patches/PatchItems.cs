    ...
    public class PatchItems
    {
        ...
        private static bool GetPreventPickup(Transform transform)
        {
            ...
            if (LeakingRadiation.main == null || !CrashedShipExploder.main.IsExploded())
            {
                ...
                return Qpatch.config.preventPreRadiativeFoodPickup;
            }
            else if (!Qpatch.config.preventRadiativeFoodPickup) // If they can pickup after ship has exploded, return early
            {
                return false;
            }
